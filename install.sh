...
echo -e "  ${BOLD}${C}╚═════╝ ╚═════╝          ╚═╝   ╚═╝  ╚═╝╚═╝  ╚═╝ ╚═════╝╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝${RESET}"
echo
echo -e "  ${DIM}Blackboard activity tracker  ·  by ${W}Franker26${RESET}  ${DIM}v1.0.30${RESET}"
echo -e "  ${DIM}──────────────────────────────────────────────${RESET}"
echo
...
if [ -d "$INSTALL_DIR/.git" ]; then
    git -C "$INSTALL_DIR" fetch -q origin main 2>/dev/null || true
    git -C "$INSTALL_DIR" reset --hard origin/main -q 2>/dev/null || true
else
...
printf "  URL de Blackboard [https://palermo.blackboard.com]: "
read -r BB_URL </dev/tty
BB_URL="${BB_URL:-https://palermo.blackboard.com}"

printf "  Usuario: "
read -r BB_EMAIL </dev/tty

printf "  Contraseña: "
read -rs BB_PASSWORD </dev/tty
echo

[ -z "$BB_EMAIL" ] && fail_step "El usuario no puede estar vacío"
[ -z "$BB_PASSWORD" ] && fail_step "La contraseña no puede estar vacía"

sed -i "s|^BB_BASE_URL=.*|BB_BASE_URL=${BB_URL}|" "$INSTALL_DIR/.env"
sed -i "s|^BB_EMAIL=.*|BB_EMAIL=${BB_EMAIL}|" "$INSTALL_DIR/.env"
sed -i "s|^BB_PASSWORD=.*|BB_PASSWORD=${BB_PASSWORD}|" "$INSTALL_DIR/.env"
echo
echo -e "  ${G}✔${RESET}  Credenciales guardadas"
echo
...
# Detener contenedores previos y limpiar volumen (DB vieja)
timeout 15 bash -c "cd \"$INSTALL_DIR\" && $COMPOSE down -v 2>/dev/null" || true
sleep 1
